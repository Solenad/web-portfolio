...
- [x] 2.3 Add Catppuccin base dim overlay (≈60% opacity) between layer and content; verify "moody midpoint" readability
...
- [x] 4.1 Rewrite `SystemInfoPanel.tsx` as `FastfetchPanel.tsx` with `roe@macchiato` ASCII banner (blue + mauve/green accents)
- [x] 4.2 Render 9 info lines in fastfetch format: padded blue labels + `text`-colored values, monospace column alignment
- [x] 4.3 Keep existing data values (OS, kernel, shell, WM, editor, terminal, theme, agents, disk) and update AboutWindow import
- [x] 4.4 Wrap fastfetch block in a glass/panel container matching the window theme

## 5. Verification

- [x] 5.1 Run `npm run lint` — no new warnings/errors from changed files
- [x] 5.2 Run `npm run build` — production build succeeds
- [ ] 5.3 Manual check: scroll window — trolley drifts subtly; glass panels keep text readable; photos still open the dark modal; XP chrome unchanged; other windows unaffected
- [ ] 5.4 Tune dim opacity / parallax factor / glass opacity to the agreed "moody midpoint"